    ...
    ui(new Ui::MiningPage)
{
    ...
    ui->serverLine->setVisible(false);
    ...
    ui->usernameLine->setVisible(false);
...
void MiningPage::minerError(QProcess::ProcessError error)
{
    ...
    {
        reportToList("Miner failed to start. Make sure you have the minerd executable and libraries in the same directory as WorldTopCoin-Qt.", ERROR, NULL);
    }
}
...
{
    if (getMiningType() == ClientModel::SoloMining)
        reportToList("Mining stopped.", ERROR, NULL);
    else
...
void MiningPage::minerStarted()
{
    if (!minerActive)
        if (getMiningType() == ClientModel::SoloMining)
            reportToList("Mining started.", ERROR, NULL);
        else
